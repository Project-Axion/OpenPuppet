...
// Taken from https://github.com/dotnet/Silk.NET/tree/main/src/OpenGL/Extensions/Silk.NET.OpenGL.Extensions.ImGui
...
#endif
{
    public class ImGuiController : IDisposable
    {
        ...
        /// </summary>
        public unsafe ImGuiController(GL gl, IWindow view, IInputContext input, ImGuiFontConfig? imGuiFontConfig = null, Action onConfigureIO = null)
        {
            Init(gl, view, input);
            ...
            var mainViewport = ImGuiNET.ImGui.GetMainViewport();

            var window = new ImGuiWindow
            {
                window = view,
                viewport = mainViewport.NativePtr,
                gl = gl,
                inputContext = _input
            };

            window.viewport.PlatformHandle = (nint)mainViewport.ID;

            ImPlatform.windows.Add(mainViewport.ID, window);
        ...
        private unsafe void Init(GL gl, IWindow view, IInputContext input)
        {
            _gl = gl;
            _view = view;
            _input = input;
            ...
            ImPlatform.platformController = this;

            pio.Platform_CreateWindow = Marshal.GetFunctionPointerForDelegate(ImPlatform._createWindow);
            pio.Platform_ShowWindow = Marshal.GetFunctionPointerForDelegate(ImPlatform._showWindow);
            pio.Platform_DestroyWindow = Marshal.GetFunctionPointerForDelegate(ImPlatform._destroyWindow);

            pio.Platform_SetWindowPos = Marshal.GetFunctionPointerForDelegate(ImPlatform._setWindowPos);
            pio.Platform_GetWindowPos = Marshal.GetFunctionPointerForDelegate(ImPlatform._getWindowPos);

            pio.Platform_SetWindowSize = Marshal.GetFunctionPointerForDelegate(ImPlatform._setWindowSize);
            pio.Platform_GetWindowSize = Marshal.GetFunctionPointerForDelegate(ImPlatform._getWindowSize);

            pio.Platform_RenderWindow = Marshal.GetFunctionPointerForDelegate(ImPlatform._renderWindow);
            pio.Platform_SwapBuffers = Marshal.GetFunctionPointerForDelegate(ImPlatform._swapBuffers);

            pio.Platform_GetWindowFocus = Marshal.GetFunctionPointerForDelegate(ImPlatform._getWindowFocus);
            pio.Platform_SetWindowFocus = Marshal.GetFunctionPointerForDelegate(ImPlatform._setWindowFocus);

            pio.Platform_GetWindowMinimized = Marshal.GetFunctionPointerForDelegate(ImPlatform._getWindowMinimized);

            pio.Platform_GetWindowMinimized = Marshal.GetFunctionPointerForDelegate(ImPlatform._getWindowMinimized);

            pio.Platform_SetWindowTitle = Marshal.GetFunctionPointerForDelegate(ImPlatform._setWindowTitle);

            pio.Platform_SetWindowAlpha = Marshal.GetFunctionPointerForDelegate(ImPlatform._setWindowAlpha);

            pio.Platform_UpdateWindow = Marshal.GetFunctionPointerForDelegate(ImPlatform._updateWindow);

            pio.Platform_GetWindowDpiScale = Marshal.GetFunctionPointerForDelegate(ImPlatform._getWindowDpi);

            pio.Platform_OnChangedViewport = Marshal.GetFunctionPointerForDelegate(ImPlatform._onViewportChange);
        ...
        private void UpdateImGuiInput()
        {
            ...
            {
                using var state = item.Value.inputContext.Mice[0].CaptureState();
        ...
        private unsafe void RenderImDrawData(GL _gl, ImDrawDataPtr drawDataPtr)
        {
            ...
            SetupRenderState(_gl, drawDataPtr, framebufferWidth, framebufferHeight);
